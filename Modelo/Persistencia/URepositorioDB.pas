...
type
  ...
  public
    ...
    //Docs TO-DO
    function RetornaRegistroFilhos<Q: TENTIDADE, constructor>
                                  (const ciValorRelacionamento: Integer;
                                   const csNomeTabelaRelacionamento: String;
                                   const csNomeDoCampoRelacionamento: String): TList<Q>;
...
const
  ...
  CNT_SELECT_BY_FIELD   = 'select * from %s where %s = :%1:s';
...
begin
  ...
  FSQLSelect.CommandText := Format(CNT_SELECT_BY_FIELD, [FNomeTabela
                                                       , FNomeCampoPK]);
...
function TRepositorioDB<T>.RetornaRegistroFilhos<Q>(
  const ciValorRelacionamento: Integer;
  const csNomeTabelaRelacionamento: String;
  const csNomeDoCampoRelacionamento: String): TList<Q>;
var
...
begin
  FSQLSelect.Close;
  FSQLSelect.CommandText := Format(CNT_SELECT_BY_FIELD,
     [csNomeTabelaRelacionamento
    , csNomeDoCampoRelacionamento]);

  FSQLSelect.Prepared    := True;
  FSQLSelect.ParamByName(csNomeDoCampoRelacionamento).AsInteger := Integer(ciValorRelacionamento);
  FSQLSelect.Open;

  Result := TList<Q>.Create;
  while not FSQLSelect.Eof do
    begin
      loENTIDADE := Q.Create;
      AtribuiDBParaEntidade(loENTIDADE);
      Result.Add(loENTIDADE);
